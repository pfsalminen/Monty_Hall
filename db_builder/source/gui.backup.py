...
class Window(qtWidget.QMainWindow):
    ...
    def init_ui(self) -> None:
        ...
        self.main_box = qtWidget.QGroupBox('Setup Options')
        ...
        self.db_name = qtWidget.QLineEdit()
        self.db_name.setPlaceholderText('DB Name')
        self.db_name.setFixedWidth(300)
        self.db_name.textEdited.connect(self._set_db)
        row_one_layout.addWidget(self.db_name)

        self.main_layout.addLayout(row_one_layout)

        row_two_layout = qtWidget.QHBoxLayout()

        self.master_name = qtWidget.QLineEdit()
        self.master_name.setPlaceholderText('Master Name')
        self.master_name.setText(self.provision.master_copy)
        self.master_name.setFixedWidth(300)
        self.master_name.textEdited.connect(self._set_master)
        row_two_layout.addWidget(self.master_name)

        self.svr_box = qtWidget.QComboBox()
        servers = [
            'DBSVR01',
            'DBSVR02',
            'DBSVR03',
        ...
        self.svr_box.addItem('------')
        for server in servers:
            self.svr_box.addItem(server)

        self.svr_box.activated[str].connect(self._set_server)
        row_two_layout.addWidget(self.svr_box)

        self.main_layout.addLayout(row_two_layout)
        self.main_layout.addWidget(QHLine(), 1, 0)
        ...
        self.go_button = qtWidget.QPushButton("Go!")
        ...
        self.final_layout = qtWidget.QVBoxLayout()

        self.progress = qtWidget.QProgressBar()
        completed = 0
        self.progress.setValue(completed)
        self.final_layout.addWidget(self.progress)

        self.info = qtWidget.QPlainTextEdit()
        # self.info.setEnabled(False)
        metrics = qtGui.QFontMetrics(FONT)
        self.info.setTabStopWidth(4 * metrics.width(' '))
        self.final_layout.addWidget(self.info)

        self.layout.addLayout(self.final_layout)
